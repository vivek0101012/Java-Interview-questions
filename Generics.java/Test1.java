...
// Type Erasure is the process by which the Java compiler removes all generic type information during compilation. The compiler replaces type parameters with their bounds or with Object if they are unbounded. Therefore, Generics provide compile-time type safety, but generic type information is not available at runtime.
...
// both list  converts to raw type list hence the same  method signature —> causes compile time issues as no two methods in same class can be over loaded with same signature .
// public
...
// Arrays require runtime type information, whereas Generics lose their type information due to Type Erasure. As a result, generic arrays would lose type safety and could lead to runtime `ClassCastException`. Therefore Java prohibits creating arrays of generic types.
